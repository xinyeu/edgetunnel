...
async function 处理叉HTTP请求(request, yourUUID, 反代上下文 = {}) {
	...
	if (isSpeedTestSite(首包.hostname, 首包.port) && 反代上下文.代理类型 === null) {
		try { reader.releaseLock() } catch (e) { }
...
function isSpeedTestSite(hostname, port) {
	...
	const 命中测速域名 = speedTestDomains.some(domain => hostname === domain || hostname.endsWith('.' + domain));
	// 仅拦截明文 HTTP(80) 测速；443 等 TLS 端口必须真实转发，否则客户端收到明文 204 导致 TLS 握手失败（测速显示 -1）
	return 命中测速域名 && Number(port) === 80;
}
	...
	await Promise.allSettled(urls.map(async (url) => {
		...
		try {
			...
			try {
				...
				// 如果所有编码都失败或无效，直接用非严格 UTF-8 解码已读取的 buffer
				//（body 已被 arrayBuffer() 消费，再调 response.text() 会抛 Body has already been used）
				if (!decodeSuccess) {
